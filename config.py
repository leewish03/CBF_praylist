...
# 구글 시트 설정 시트 상수
# ===========================
COMMON_PRAYERS_SHEET = '설정_공통기도제목'  # 공통 기도제목 설정 시트명
ASSIGNMENTS_SHEET = '설정_담당자배정'        # 담당자 배정 설정 시트명
...
@dataclass  
class NotionConfig:
    """Notion 관련 설정 (선택적 - API 서버 단독 동작 가능)"""
    token: str
    page_id: str
...
class AppConfig:
    ...
    def validate(self, require_notion: bool = True):
        """
        설정 유효성을 검증합니다
        
        Args:
            require_notion: Notion 설정 필수 여부 (기본값: True)
                           API 서버 단독 동작 시 False로 설정 가능
        """
        errors = []
    ...
    def validate_for_api_server(self):
        """
        API 서버 단독 동작을 위한 최소 설정 검증
        (Notion 설정 없이도 동작 가능)
        """
        self.validate(require_notion=False)
